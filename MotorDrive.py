...
class motorDrive:
	def __init__(self):
		...
		GPIO.setup("P9_12", GPIO.OUT) #IN1
		GPIO.setup("P9_11", GPIO.OUT) #IN2
		GPIO.setup("P9_13", GPIO.OUT) #IN3
		GPIO.setup("P9_15", GPIO.OUT) #IN4

		#sets up GPIO pins for motors 3 and 4
		GPIO.setup("P8_14", GPIO.OUT) #IN1
		GPIO.setup("P8_10", GPIO.OUT) #IN2
		GPIO.setup("P8_16", GPIO.OUT) #IN3
		GPIO.setup("P8_18", GPIO.OUT) #IN4
		
		#initilizes PW
		PWM.start("P9_16", 0) #ENA 1
		PWM.start("P9_14", 0) #ENB 2
		PWM.start("P8_13", 0) #ENA 3
		PWM.start("P8_19", 0) #ENB 4
		
		
	def leftFront(self, motorspeed):
	#motorspeed -100-100,

		#values may not be correct for direction
		if motorspeed < 0:
				Channel_A = 1
				Channel_B = 0
		elif motorspeed > 0:
		...
		#left Side
		#writes to motor 1
		GPIO.output("P9_12", Channel_A)
		GPIO.output("P9_11", Channel_B)
